...
class FeatureLoss(nn.Module):
    ...
    def forward(
        self, latents_per_period: list[torch.Tensor], latents_per_period_gt: list[torch.Tensor]
    ):
        loss = 0
        for cur_latents, cur_latents_gt in zip(latents_per_period, latents_per_period_gt):
            for latent, latent_gt in zip(cur_latents, cur_latents_gt):
                loss += torch.mean(torch.abs(latent - latent_gt)) / torch.prod(
                    torch.tensor(latent.shape[1:])
                )
    ...
    def forward(
        ...
        **kwargs,
        ...
        total_gen_loss = (
            mpd_gen_loss
            + msd_gen_loss
            + self.feature_loss_coeff * (mpd_feature_loss + msd_features_loss)
            + self.melspec_loss_coeff * melspec_loss
        )
